...
data = []
csv_path = './indicators/data/2017-01-27-ql-resources.csv'
csv_path = './indicators/data/sample.csv'
...
def relative_strength_index(data, days=14):
  ...
  for index, value in enumerate(skipped_first_value):
    if index + 1 > days:
      ...
      average_gain = abs(sum([change for change in skipped_first_value[:days] if change > 0])) / 14
      average_loss = abs(sum([change for change in skipped_first_value[:days] if change < 0])) / 14
    try:
